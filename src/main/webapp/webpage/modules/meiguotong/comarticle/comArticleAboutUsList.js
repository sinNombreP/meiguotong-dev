...
$(document).ready(function() {
	$('#comArticleTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/meiguotong/comarticle/comArticle/dataUs",
		    ...
		    , {
				...
				events:{
   		         'click .delete': function (e, value, row, index) { 
 		        	jp.confirm('确定删除关于我们信息吗？', function(){
                        	jp.loading();
                        	jp.get("${ctx}/meiguotong/comarticle/comArticle/delete?id="+row.id, function(data){
                    	  		if(data.success){
                    	  			$('#comArticleTable').bootstrapTable('refresh');
 		        ...
 		        'click .status1': function (e, value, row, index) { 
 		        	jp.confirm('确定启用关于我们信息吗？', function(){
                        	jp.loading();
                        	jp.get("${ctx}/meiguotong/comarticle/comArticle/status?id="+row.id, function(data){
                    	  		if(data.success){
                    	  			$('#comArticleTable').bootstrapTable('refresh');
                    	  			jp.success(data.msg);
                    	  		}else{
 		        ...
 		        'click .status2': function (e, value, row, index) { 
 		        	jp.confirm('确定禁用关于我们信息吗？', function(){
                        	jp.loading();
                        	jp.get("${ctx}/meiguotong/comarticle/comArticle/status?id="+row.id, function(data){
                    	  		if(data.success){
                    	  			$('#comArticleTable').bootstrapTable('refresh');
                ...
                formatter:  function operateFormatter(value, row, index) {
    		        return [
						<shiro:hasPermission name="meiguotong:comarticle:comArticle:edit"> 
							...
							row.status==2?'<a href="#" class="status1"><i class="fa fa-edit btn btn-success btn-xs">启用</i> </a>':'',
							row.status==1?'<a href="#" class="status2"><i class="fa fa-edit btn btn-danger btn-xs">禁用</i> </a>':'',
    		        ].join('');
    		    }
            }
		     ]
  ...
  function edit(id){
	  window.location = "${ctx}/meiguotong/comarticle/comArticle/aboutUsForm?flag=1&id="+id;
  }
  function add(){
	  window.location = "${ctx}/meiguotong/comarticle/comArticle/aboutUsForm?flag=1";
  }
  function view(id){
	  window.location = "${ctx}/meiguotong/comarticle/comArticle/aboutUsForm?flag=2&id="+id;
  } 
</script>