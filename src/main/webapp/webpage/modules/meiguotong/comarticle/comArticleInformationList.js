...
$(document).ready(function() {
	$('#comArticleTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/meiguotong/comarticle/comArticle/InformationData",
                ...
                formatter:  function operateFormatter(value, row, index) {
    		        return [
    		        	<shiro:hasPermission name="meiguotong:comarticle:comArticle:edit"> 
							'<a id="edit" class="view" onclick="edit('+row.id+')"><i class="fa fa-eye btn btn-success btn-xs">编辑</i> </a>',
						</shiro:hasPermission>
					(row.status==1?'<a href="#" class="status" title="禁用"><i class="fa fa-edit btn btn-warning btn-xs">禁用</i> </a>':
					(row.status==2?'<a href="#"  class="status" title="正常"><i class="fa fa-edit btn btn-success btn-xs">启用</i> </a>':'')),
    		        ].join('');
    		    }
            }
		     ]
  ...
  function edit(id){
	  window.location = "${ctx}/meiguotong/comarticle/comArticle/Informationform?id=" + id;
  }
  function add(){
	  window.location = "${ctx}/meiguotong/comarticle/comArticle/Informationform?id=";
  }
</script>