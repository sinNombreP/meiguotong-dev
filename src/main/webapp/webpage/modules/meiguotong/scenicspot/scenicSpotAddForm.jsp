...
<head>
	...
	background-color:#0cf;}
	   .btn_add_pic_a,.btn_add_pic_b,.btn_add_pic_c,.btn_add_pic_d{  
		...
		var btnCount_c=0; 
		var btnCount_d=0;  
	    function initClickListener_a(){  
	        $(".btn_add_pic_a").click(function(){
	        	if(btnCount_a>0){
	        		if($("#file_a_"+btnCount_a).length>0){
		        		var fileValue=$("#file_a_"+btnCount_a).val();
	            ...
	            btnCount_a++;  
	            var t='<div class="dv_pic_item_a">';  
	            t+='<img src="" alt="" class="img_style" id="img_a_'+btnCount_a+'"/>';  
	            t+='<input type="hidden" name="imgUrl" class="imgSrc" id="src_a_'+btnCount_a+'"/>';  
	            t+='<input type="file" class="input_file_style_a" id="file_a_'+btnCount_a+'"/>';
	            t+='<a id="btn_a_'+btnCount_a+'" onclick="deleteElement(this)" class="deleteBtn">删除</a>'
	            t+='</div>';  
	            $(".dv_pic_box_a").append(t);
	        });
		};  
		function initClickListener_b(){  
		 ...
		 function initClickListener_c(){  
		        $(".btn_add_pic_c").click(function(){
		        	if(btnCount_c>0){
		        		if($("#file_c_"+btnCount_c).length>0){
			        		var fileValue=$("#file_c_"+btnCount_c).val();
				        	if(fileValue == null || fileValue==""){
				        		return;
				        	}
		        		}
		            ...
		            btnCount_c++;  
		            var t='<div class="dv_pic_item_c">';  
		            t+='<img src="" alt="" class="img_style" id="img_c_'+btnCount_c+'"/>';  
		            t+='<input type="hidden" name="imgUrl" class="imgSrc" id="src_c_'+btnCount_c+'"/>';  
		            t+='<input type="file" class="input_file_style_c" id="file_c_'+btnCount_c+'"/>';
		            t+='<a id="btn_c_'+btnCount_c+'" onclick="deleteElement(this)" class="deleteBtn">删除</a>'
		            t+='</div>';  
		            $(".dv_pic_box_c").append(t);
		        });
			}; 
			 function initClickListener_d(){  
			        $(".btn_add_pic_d").click(function(){
			        	if(btnCount_d>0){
			        		if($("#file_d_"+btnCount_d).length>0){
				        		var fileValue=$("#file_d_"+btnCount_d).val();
					        	if(fileValue == null || fileValue==""){
					        		return;
					        	}
			        		}
			            ...
			            btnCount_d++;  
			            var t='<div class="dv_pic_item_d">';  
			            t+='<img src="" alt="" class="img_style" id="img_d_'+btnCount_d+'"/>';  
			            t+='<input type="hidden" name="imgUrl" class="imgSrc" id="src_d_'+btnCount_d+'"/>';  
			            t+='<input type="file" class="input_file_style_d" id="file_d_'+btnCount_d+'"/>';
			            t+='<a id="btn_d_'+btnCount_d+'" onclick="deleteElement(this)" class="deleteBtn">删除</a>'
			            t+='</div>';  
			            $(".dv_pic_box_d").append(t);
			        });
				}; 
		function uploadComplete_a(evt) {
			var textJson = JSON.parse(evt.target.responseText);
			if(textJson.success){
				$(".dv_pic_item_a").eq(divIndex).children(".imgSrc").val(textJson.body.filePath);
				$(".dv_pic_item_a").eq(divIndex).children(".img_style").attr("src",src);
			} 
		...
		function uploadComplete_c(evt) {
			var textJson = JSON.parse(evt.target.responseText);
			if(textJson.success){
				$(".dv_pic_item_c").eq(divIndex).children(".imgSrc").val(textJson.body.filePath);
				$(".dv_pic_item_c").eq(divIndex).children(".img_style").attr("src",src);
			} 
			jp.success(textJson.msg);//这是框架方法非框架用alert
		}
		function uploadComplete_d(evt) {
			var textJson = JSON.parse(evt.target.responseText);
			if(textJson.success){
				$(".dv_pic_item_d").eq(divIndex).children(".imgSrc").val(textJson.body.filePath);
				$(".dv_pic_item_d").eq(divIndex).children(".img_style").attr("src",src);
			} 
			jp.success(textJson.msg);//这是框架方法非框架用alert
		}
		...
		$(document).ready(function() {
			...
			//点击添加多图——a
			initClickListener_a();
			$(".dv_pic_box_a").on("change",".input_file_style_a",function(e){ 
		        	var fileId=$(this).attr("id");
		        	divIndex=$(".dv_pic_item_a").index( $("#"+fileId).parents(".dv_pic_item_a"))
		        	var target=document.getElementById($(this).attr("id"));
		        	if(!checkImg(target)){
		      			return;
		      		}   
		         	 for (var i = 0; i < e.target.files.length; i++) {    
		            	  ...
		            	  freader.onload = function(e) { 
		            			...
		            			var fd = new FormData();
		              			fd.append("attach", file);
		              			var xhr = new XMLHttpRequest();
		              			xhr.addEventListener("load", uploadComplete_a, false);
		              			xhr.addEventListener("error", uploadFailed, false);
		              			xhr.addEventListener("abort", uploadCanceled, false);
		              			xhr.open("POST", "${ctx}/meiguotong/scenicspot/scenicSpot/uploadFile");
		              			xhr.send(fd);
			...
			//点击添加多图——c
			initClickListener_c();
			$(".dv_pic_box_c").on("change",".input_file_style_c",function(e){ 
		        	var fileId=$(this).attr("id");
		        	divIndex=$(".dv_pic_item_c").index( $("#"+fileId).parents(".dv_pic_item_c"))
		        	var target=document.getElementById($(this).attr("id"));
		        	if(!checkImg(target)){
		      			return;
		      		}   
		         	 for (var i = 0; i < e.target.files.length; i++) {    
		            	  ...
		            	  freader.onload = function(e) { 
		            			...
		            			var fd = new FormData();
		              			fd.append("attach", file);
		              			var xhr = new XMLHttpRequest();
		              			xhr.addEventListener("load", uploadComplete_c, false);
		              			xhr.addEventListener("error", uploadFailed, false);
		              			xhr.addEventListener("abort", uploadCanceled, false);
		              			xhr.open("POST", "${ctx}/meiguotong/scenicspot/scenicSpot/uploadFile");
		              			xhr.send(fd);
			...
			//点击添加多图——d
			initClickListener_d();
			$(".dv_pic_box_d").on("change",".input_file_style_d",function(e){ 
		        	var fileId=$(this).attr("id");
		        	divIndex=$(".dv_pic_item_d").index( $("#"+fileId).parents(".dv_pic_item_d"))
		        	var target=document.getElementById($(this).attr("id"));
		        	if(!checkImg(target)){
		      			return;
		      		}   
		         	 for (var i = 0; i < e.target.files.length; i++) {    
		            	  ...
		            	  freader.onload = function(e) { 
		            			...
		            			var fd = new FormData();
		              			fd.append("attach", file);
		              			var xhr = new XMLHttpRequest();
		              			xhr.addEventListener("load", uploadComplete_d, false);
		              			xhr.addEventListener("error", uploadFailed, false);
		              			xhr.addEventListener("abort", uploadCanceled, false);
		              			xhr.open("POST", "${ctx}/meiguotong/scenicspot/scenicSpot/uploadFile");
		              			xhr.send(fd);
    ...
    <script type="text/javascript">
    var ue = UE.getEditor('reserveInfo');//获得去机器
    var ue1 = UE.getEditor('introduce');//获得去机器
	...
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title"> 
				<a class="panelButton" href="${ctx}/meiguotong/scenicspot/scenicSpot"><i class="ti-angle-left"></i> 返回</a>
			</h3>
		</div>
		...
		<c:if test="${fns:hasPermission('meiguotong:scenicspot:scenicSpot:edit') || isAdd}">
