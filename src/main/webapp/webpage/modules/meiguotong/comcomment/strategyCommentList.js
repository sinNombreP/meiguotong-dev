...
$(document).ready(function() {
	$('#comCommentTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        sortable: true,
		        formatter:function(value,row,index){
			    	 if(value==0) return '<button type="button" class="btn btn-primary">禁用</button>';
			         if(value==1) return '<button type="button" class="btn btn-danger">正常</button>';
		        }
		    ...
		    , {
                ...
                formatter:  function operateFormatter(value, row, index) {
					...
					row.status==0?'<a href="#"  class="status" title="显示"><i class="fa fa-edit btn btn-danger btn-xs">启用</i> </a>':''
    		        ].join('');
    		    }
            }
		     ],
                       ...
                       columns : [                          
               		    ...
               		    , {
                               ...
                               formatter:  function operateFormatter(value, row, index) {
                   		        return [
               					row.status==1?'<a href="#" class="status" title="禁用"><i class="fa fa-edit btn btn-warning btn-xs">禁用</i> </a>':'',
               					row.status==0?'<a href="#"  class="status" title="显示"><i class="fa fa-edit btn btn-danger btn-xs">显示</i> </a>':''
                   		        ].join('');
                   		    }
                           }
                       ],
                     //  url: "${ctx}/meiguotong/comcomment/comComment/getCommentDate",
                       data:row.comCommentList,
                       method : 'POST', //请求方式(*)
  ...
  function view(id){
	  window.location = "${ctx}/meiguotong/comcomment/comComment/childComment?typeId=" + id;
  }
  function back(id){
  	   window.location = "${ctx}/meiguotong/citystrategy/cityStrategy?cityId=" + id;
  }
</script>