		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/meiguotong/hotelroomdevice/hotelRoomDevice/save",$('#inputForm').serialize(),function(data){
						if(data.success){							
...
<body class="bg-white">
		<form:form id="inputForm" modelAttribute="hotelRoomDevice" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered">
		   <tbody>
		   		<tr>
					...
					<c:choose>
						<c:when test="${hotelRoomDevice.id!=null && hotelRoomDevice.id!=''}">
					...
					<td>
						<div class="dv_info_box">  
	       					 <div class="dv_pic_box">
										...
										<img alt="暂无图片" src="${hotelRoomDevice.icon}" height="80px" width="100px" class="img_style" id="img"/> 
						            	<input type="hidden" name="icon" value="${hotelRoomDevice.icon}" class="imgSrc"/>   
