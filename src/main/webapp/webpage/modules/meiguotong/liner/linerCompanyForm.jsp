		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/meiguotong/liner/linerCompany/save",$('#inputForm').serialize(),function(data){
						if(data.success){
...
<body class="bg-white">
		<form:form id="inputForm" modelAttribute="linerCompany" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered">
		   <tbody>
				<tr>
					...
					<td class="width-35"> 
					<c:if test="${empty linerCompany.id}">
					...
					<c:if test="${not empty linerCompany.id}">
						<input value="${linerCompany.languageName}" htmlEscape="false"  readonly="readonly"  class="form-control "/>
					</c:if>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">邮轮公司名称：</label></td>
					<td class="width-35">
