...
$(document).ready(function() {
	$('#orderLinerTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/meiguotong/orderliner/orderLiner/AfterSaleData",
                ...
                events: {                
                	'click .no': function (e, value, row, index) { 
    		        	jp.confirm('确认拒绝退款吗？', function(){
                           	jp.loading();
                           	jp.get("${ctx}/meiguotong/orderliner/orderLiner/status?status=10&id="+row.id, function(data){
                       	  		if(data.success){
                       	  			$('#orderLinerTable').bootstrapTable('refresh');
                       	  			jp.success(data.msg);
                       	  		}else{
			...
			'click .yes': function (e, value, row, index) { 
	        	jp.confirm('确认同意退款吗？', function(){
                   	jp.loading();
                   	jp.get("${ctx}/meiguotong/orderliner/orderLiner/status?status=8&id="+row.id, function(data){
               	  		if(data.success){
               	  			$('#orderLinerTable').bootstrapTable('refresh');
               	  			jp.success(data.msg);
               	  		}else{
	           ...
	           'click .sure': function (e, value, row, index) { 
		        	jp.confirm('确认退款吗？', function(){
	                   	jp.loading();
	                   	jp.get("${ctx}/meiguotong/orderliner/orderLiner/status?status=9&id="+row.id, function(data){
	               	  		if(data.success){
	               	  			$('#orderLinerTable').bootstrapTable('refresh');
	               	  			jp.success(data.msg);
	               	  		}else{
