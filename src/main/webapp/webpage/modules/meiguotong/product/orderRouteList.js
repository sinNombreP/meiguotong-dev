...
$(document).ready(function() {
	...
	if ($("#orderType").val()==1) {
		...
		$('#orderRouteTable').bootstrapTable({
			  ...
			  //请求方法
	               ...
	               url: "${ctx}/meiguotong/order/orderRoute/data",
	               ...
	               onContextMenuItem: function(row, $el){
	                   if($el.data("item") == "edit"){
	                   	window.location = "${ctx}/meiguotong/order/orderRoute/form?id=" + row.id;
	                   } else if($el.data("item") == "delete"){
	                        jp.confirm('确认要删除该参团订单记录吗？', function(){
	                       	jp.loading();
	                       	jp.get("${ctx}/meiguotong/order/orderRoute/delete?id="+row.id, function(data){
	                   	  		if(data.success){
	                   	  			$('#orderRouteTable').bootstrapTable('refresh');
	                   	  			jp.success(data.msg);
	                   	  		}else{
	                   	  			jp.error(data.msg);
	                   	  		}
				...
				,{
			        ...
			        formatter:function(value,row,index){
				    	return (value!=null?value:"")+(row.contactsMobile!=null?","+row.contactsMobile:"")+(row.remark!=null?","+row.remark:"")
			        }
			    ...
			    , {
	                ...
	                formatter:  function operateFormatter(value, row, index) {
	    		        return [
							'<a href="${ctx}/meiguotong/order/orderRoute/form?type='+$("#type").val()+'&id='+row.id+'" class="view" title="查看" >'+
	...
	function sureOrder(row){
		jp.confirm('是否确定订单？', function(){
          	jp.loading();
          	jp.get("${ctx}/meiguotong/order/orderRoute/sureOrder?id="+row.id, function(data){
      	  		if(data.success){
      	  			$('#orderRouteTable').bootstrapTable('refresh');
      	  			jp.success(data.msg);
      	  		}else{
	...
	function cancelOrder(row){
		jp.confirm('确认取消订单吗？', function(){
			jp.loading();
			jp.get("${ctx}/meiguotong/order/orderRoute/cancelOrder?id="+row.id, function(data){
				if(data.success){
					$('#orderRouteTable').bootstrapTable('refresh');
					jp.success(data.msg);
				}else{
	...
	function refundOrder(row){
		jp.confirm('确认申请退款吗？', function(){
			jp.loading();
			jp.get("${ctx}/meiguotong/order/orderRoute/refundOrder?id="+row.id, function(data){
				if(data.success){
					$('#orderRouteTable').bootstrapTable('refresh');
					jp.success(data.msg);
				}else{
	...
	function agreeOrder(row){
		jp.confirm('确定同意退款吗？', function(){
			jp.loading();
			jp.get("${ctx}/meiguotong/order/orderRoute/agreeOrder?id="+row.id, function(data){
				if(data.success){
					$('#orderRouteTable').bootstrapTable('refresh');
					jp.success(data.msg);
				}else{
	...
	function confirmorder(row){
		jp.confirm('确定退款吗？', function(){
			jp.loading();
			jp.get("${ctx}/meiguotong/order/orderRoute/confirmorder?id="+row.id, function(data){
				if(data.success){
					$('#orderRouteTable').bootstrapTable('refresh');
					jp.success(data.msg);
				}else{
	...
	function refuseOrder(row){
		jp.confirm('确定拒绝退款吗？', function(){
			jp.loading();
			jp.get("${ctx}/meiguotong/order/orderRoute/refuseOrder?id="+row.id, function(data){
				if(data.success){
					$('#orderRouteTable').bootstrapTable('refresh');
					jp.success(data.msg);
				}else{
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#orderRouteTable').bootstrapTable("toggleView");
		}
	  
	  $('#orderRouteTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#remove').prop('disabled', ! $('#orderRouteTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#orderRouteTable').bootstrapTable('getSelections').length!=1);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/meiguotong/order/orderRoute/import/template';
				  },
  ...
  function getIdSelections() {
        return $.map($("#orderRouteTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该参团订单记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/meiguotong/order/orderRoute/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#orderRouteTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/meiguotong/order/orderRoute/form?id=" + getIdSelections();
  }
