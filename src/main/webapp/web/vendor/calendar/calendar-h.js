...
(function () {
                ...
                if (flag) {
                    riliHtml += '' +
                        '<div class="ht-rili-td ht-rili-onclick" data-date="' + dataDateStr +'">' +
                            '<span class="ht-rili-day">' + (k + 1) +'</span>' +
    ...
    function getTotalPrice() {
        var totalPrice = parseFloat($(".ht-rili-td-active").children(".ht-rili-money").attr("data-money"));//价格
    ...
    $.fn.extend({
        calendar: function (opt) {
            ...
            $('.ht-rili-leftarr').on('click', function () {
                ...
                $('.ht-rili-date').html(calendarDate.year + '年 ' + calendarDate.month + '月');
                getIndexDay();
                app.clickType == 1? app.getGuideDateDetails():reservation();
            });
