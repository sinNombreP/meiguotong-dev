...
(function ($) {
  ...
  fn._createDaySetupInputGroup = function () {
    ...
    html += '<li>';
    html += '   <label></label>';
    html += '   <label>基本价格</label> ';
    html += '   <label>库存</label>';
    html += '</li>';
    for (var i = 0; i < config.length; i++) {
      var val = config[i];
      html += '<li>';
      html += '   <label>'+ val.titleCh +'</label>';
      html += '   <input name="'+ val.id +'" type="text" value="'+ val.price +'">';
      html += '   <input name="'+ val.id +'" type="text" value="'+ val.dateaccount +'">';
      html += '</li>';
    }
    return html;
  };
