...
(function ($) {
  ...
  fn.initSettingWindow = function () {
    ...
    html += '<div class="cddsw-container" style="max-height:600px;">';
    html += '   <div class="cddsw-head-wrapper">';
    html += '       <div class="cddsw-title">0000-00-00</div>';
    html += '       <a class="cddsw-close"><i></i></a>';
    html += '   </div>';
    html += '   <table style="margin-left:20px" class="cddsw-form-wrapper datetable" id="table1" >';
    html += '   </table>';
    html += '   <table style="margin-left:20px" class="cddsw-form-wrapper datetable" id="table2" >';
    html += '   </table>';
   
    html += '   <div class="cddsw-foot-wrapper ">';
    html += '       <button class="btn-confirm">保存</button>';
    html += '       <button class="btn-back">还原</button>';
  ...
  fn.handleClickEvent = function () {
    ...
    this.calendar.on('click', '.valid-hook', function () {
	      var productid = $("#productid").val();
	        ...
	        var u = [];  
	        var attru = [];
	        $.post("/a/sys/productDate/formDetail",{productid:productid,date:thisDate},function(data){
	          var result=data.basics;
	          var attrs=data.attrs;
	          console.log("attrs.length:"+attrs.length);
	          console.log("attrs"+attrs);
	              for(var i=0;i<result.length;i++){ 
	            	  var basic={};
	            	  basic.id=result[i].id;
	            	  basic.price=result[i].price;
	            	  basic.titleCh=result[i].titleCh;
	            	  basic.dateaccount = result[i].dateaccount;
	            	  basic.datebasid=result[i].datebasid;
	            	  u.push(basic);
	              };
	              // 用户传入字段
	    	      $("#table1").empty();
	    	      ...
	    	      html += '<td><label>基本价格</label></td> ';
	    	      html += '<td><label>库存</label></td>';
	    	      html += '</tr>';

	    	      for (var i = 0; i < config.length; i++) {
	    	        var val = config[i];
	    	        html += '<tr>';
	    	        html += '<td><input type="hidden"  name="id" value='+ val.id +'><label>'+ val.titleCh +'</label></td>';
	    	        html += '<td><input type="text" name="price" value='+ val.price +'></input></td>';
	    	        html += '<td><input type="text" name="dateaccount"  value='+ val.dateaccount +'></input></td>';
	    	        html += '<td style="display:none"><input type="hidden"  name="datebasid" value='+ val.datebasid +'></td>';
	    	        html += '</tr>';
	    	      }
	              ...
	              for(var i=0;i<attrs.length;i++){   
	            	  html2 += '<tr>';
		    	      html2 += '<td><label>'+attrs[i].specname+'</label></td>';
		    	      html2 += '<td><label>叠加价格</label></td> ';
		    	      html2 += '<td><label>库存</label></td>';
		    	      html2 += '</tr>';
		    	      var specAttrs = attrs[i].specAttrs;
		    	      if(specAttrs!=null && specAttrs.length>0){
		    	    	  for(var j=0;j<specAttrs.length;j++){
		    	    		var val1 = specAttrs[j];
		  	    	        console.log(val1);
		  	    	      var productid=$("#productid").val();
		  	    	        html2 += '<tr>';
		  	    	        html2 += '<td><input type="hidden"  name="id" value='+ productid +'><label>'+ val1.attrnameCh +'</label></td>';
		  	    	        html2 += '<td><input type="text" value='+ val1.price +'></input></td>';
		  	    	        html2 += '<td><input type="text"  value='+ val1.stock +'></input></td>';
		  	    	        html2 += '<td style="display:none"><input type="hidden"  name="dateattrid" value='+ val1.dateattrid +'></td>';
		  	    	        html2 += '<td style="display:none"><input type="hidden"  name="specid" value='+attrs[i].prospid+'></td>';
		  	    	        html2 += '<td style="display:none"><input type="hidden"  name="attrid" value='+ val1.attrid +'></td>';
		  	    	        html2 += '</tr>';
		    	    	  }
		    	      }
    ...
    // 还原设置
    this.settingWindow.on('click', '.btn-back', function () {
    	console.log("back");
    	me.settingWindow.hide();
        ...
        var productid=$("#productid").val();
        $.ajax({  
            url: "/a/sys/productDate/deleteDateResult",  
            data: {productid:productid,date:thisDate},//此处data为上传的数据  
            dataType: "json",  
            type: "POST",
        ...
        location.href="/a/sys/productDate/date?productid="+$("#productid").val();
    });
    ...
    this.settingWindow.on('click', '.btn-confirm', function () {
      ...
      $("#table1").find("tr").each(function(){	  
          ...
          var dateaccount = tdArr.eq(2).find('input').val();
          var datebasid = tdArr.eq(3).find('input').val();
          ...
          if(id!=null && typeof(id)!="undefined"){
        	  ...
        	  basic.bacid=id;
        	  basic.price= price;
        	  basic.stock= dateaccount;
        	  console.log(basic+"==>");
        	  basics.push(basic);
          }
          console.log(id+"==>"+price+"==>"+dateaccount);
      });
      ...
      $("#table2").find("tr").each(function(){	  
          ...
          var id = tdArr.eq(0).find('input').val();
          var price = tdArr.eq(1).find('input').val();
          var stock = tdArr.eq(2).find('input').val();
          var dateattrid = tdArr.eq(3).find('input').val();
          var specid = tdArr.eq(4).find('input').val();
          var attrid = tdArr.eq(5).find('input').val();
          if(id!=null && typeof(id)!="undefined"){
        	  var attr={};
        	  if(dateattrid!="null"){
        		  attr.id =dateattrid;
      ...
      console.log("sttrs:"+JSON.stringify(attrs));
      var productid=$("#productid").val();
      $.ajax({  
          url: "/a/sys/productDate/saveDateResult",  
          data: {basics:JSON.stringify(basics),attrs:JSON.stringify(attrs),productid:productid,date:thisDate},//此处data为上传的数据  
          dataType: "json",  
          type: "POST",
          success: function (msg) {  
              console.log(msg);  
      ...
      $dateSetWrapper.find('.cddsw-form-wrapper [name]').each(function () {
        ...
        if(key.toString() == "price"){
          ...
          if(setData["price"] ==null || setData["price"]> val){
            setData[key] = val;
          }
        }
        if(key.toString() == "dateaccount"){
          var val ;
          if($(this).val()==null){
            val=0; 
          }else{
          ...
          if(setData["dateaccount"] ==null || setData["dateaccount"]> val){
            setData[key] = val;
          }
        }
        console.log(key + ' => ' + val);
