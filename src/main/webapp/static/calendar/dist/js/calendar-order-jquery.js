...
(function ($) {
  ...
  fn._prevMonth = function () {
    ...
    var dateList=[];
    var orderDate =formatDate(this.month, 'yyyy-MM');
    var url=$("#requsturl").val();
    $.post(url+"/meiguotong/car/carService/JourneyOfCar?date="+orderDate+"&carid="+$("#carid").val(),{suggest:"txt"},function(result){
...
//    }
  ...
  fn.renderDataToTalbe = function () {
    ...
    this.calendar.find('.valid-hook').each(function () {
      ...
      if (dayData) {
        ...
        for (var key2 in dayData) {
            html = html.replace(key2, dayData[key2]);
          }
  ...
  fn.dayComplate = function () {
    ...
    if (arr && arr instanceof Array) {
      for (var i = 0; i < arr.length; i++) {
        var val = arr[i];
        html += '<p class="val'+i+'"><input id="orderCarid" type="hidden" value="'+val.key2+'">{'+ val.key +'}'+ val.name +'</p>'
      }
    }
    return html;
  };
 ...
 // 点击
    this.calendar.on('click', '.val0', function () {
    var thisDate = $(this).parent().parent().data('id');//当前所点击的日期
     var url=$("#requsturl").val();
     //alert($(this).children().val());
     //top.openTab('${ctx}/meiguotong/agentextract/agentExtract/viewOrder?id=1','订单列表',false) 
     jp.openTab(url+"/meiguotong/car/carService/orderCarInfo?id="+$(this).find("input").val(),"订单详情",true);
 	 //jp.openDialog('订单详情', "${ctx}/meiguotong/agentextract/agentExtract/viewOrder?id=1",'1000px', '500px', $('#agentExtractUnExtractTable'));
