...
public class Global {
	 ...
	 * 保存全局属性值
	 */
	private static Map<String, String> payMap = Maps.newHashMap();
	 ...
	 * 支付文件加载对象
	 */
	private static PropertiesLoader payLoader = new PropertiesLoader("/properties/pay.properties");
	 ...
	 * @see ${fns:getConfig('adminPath')}
	 */
	public static String getPayConfig(String key) {
		String value = payMap.get(key);
		if (value == null){
			value = payLoader.getProperty(key);
			payMap.put(key, value != null ? value : StringUtils.EMPTY);
		}
		return value;
	}
